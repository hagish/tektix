...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 pixel_coords){
	...
	float red = Texel(texture, vec2(texture_coords.x + offsetX, texture_coords.y + pSize.y * 0.25)).r;
	float green = Texel(texture, vec2(texture_coords.x + offsetX, texture_coords.y - pSize.y * 0.25)).g;
	...
	if(mod(texture_coords.y * screen.y, 2.0) > 0.5) {
		...
		return vec4(vec3(red * 0.8, green * 0.8, blue * 0.8) * brightness, 1.0);
	}
}